...
切片操作：b = a[:] 或者列表生成式 b = [x for x in a]；
工厂函数：b = list(a)；
copy 函数：b = copy.copy(a)；
要注意的是，浅拷贝之所以称之为浅拷贝，是它仅仅只拷贝了一层，在列表 a 中有一个嵌套的 list，如果我们修改了它，情况就不一样了。  
修改后，copy变量b对新对象引用不会改变,只有b的内容改变了。  
浅拷贝后其中内部多层嵌套的引用不会发生改变。

### 深拷贝拷贝了对象中所有元素，包括多层嵌套列表中的元素。因此他的时间成本和空间成本开销要高。
因为深拷贝拷贝出来的对象根本就是一个全新的对象，不再与原来的对象有任何的关联。
## init和new的区别
